  ...
  private def validate_node(node, pool, generated_hostnames, errors, warnings)
    ...
    unless node.manage_hostname
      begin
        ...
        if generated_hostnames.includes?(hostname)
          errors << "External node #{node.host} has hostname '#{hostname}' which conflicts with a hostname generated by hetzner-k3s. Set manage_hostname: true or change the node's hostname."
        end
      rescue ex
